Shader "Custom/TriplanarPixelized"
{
    Properties
    {
        ...
        _UseSpecularMap("Use Spec map",Range(0,1)) = 0
    ...
    SubShader
    {
        ...
        void surf (Input IN, inout SurfaceOutputStandard o)
        {
            ...
            if (abs(localNormal.x) >= max(abs(localNormal.y), abs(localNormal.z)))
            {
                ...
                normalPixel = UnpackNormal((tex2D(_NormalMap, texPos)));
            ...
            o.Albedo = (tex2D(_MainTex, texPos) * _Color).rgb;

            float smoothness = _Glossiness;
            if (_UseSpecularMap > 0)
