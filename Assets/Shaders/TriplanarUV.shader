Shader "Custom/TriplanarUV"
{
    Properties
    {
        ...
        _UVNormalMap ("UV NormalMap", 2D) = "gray" {}
        _UVSpecularMap ("UV SpecularMap", 2D) = "gray" {}
    ...
    SubShader
    {
        ...
        sampler2D _UVTex;
        sampler2D _UVNormalMap;
        sampler2D _UVSpecularMap;
        ...
        void surf (Input IN, inout SurfaceOutputStandard o)
        {
            ...
            float4 albedo = tex2D(_UVTex, IN.uv_MainTex);
            float smoothness = 0;

            if (albedo.a > 0.05f)
            {
                normalPixel = UnpackNormal(tex2D(_UVNormalMap, IN.uv_MainTex));
                smoothness = tex2D(_UVSpecularMap, IN.uv_MainTex);
            }
            else
            {
                ...
                texPos *= _Scale;
                albedo = (tex2D(_MainTex, texPos) * _Color);
                smoothness = (tex2D(_SpecularMap, texPos)).r;
            }
